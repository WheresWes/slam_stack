...
// Cross-platform threading support
    ...
    // pthread compatibility layer for Windows
    ...
    inline int pthread_mutex_init(pthread_mutex_t* mutex, void* attr) {
        (void)attr;
        // std::mutex is already initialized by default constructor
        return 0;
    }
    ...
    inline int pthread_mutex_destroy(pthread_mutex_t* mutex) {
        (void)mutex;
        // std::mutex destructor handles cleanup
        return 0;
    }

    inline int pthread_mutex_trylock(pthread_mutex_t* mutex) {
        return mutex->try_lock() ? 0 : 1;
    }
