/**
 * @file use_ikfom.hpp
 * @brief EKF state manifold definition for FAST-LIO
 *
 * Defines the state, input, and noise manifolds used by the
 * Iterated Extended Kalman Filter on manifolds (IKFoM).
 */

#ifndef USE_IKFOM_HPP
#define USE_IKFOM_HPP

#include "slam/IKFoM_toolkit/esekfom/esekfom.hpp"
...
/**
 * State manifold for SLAM
 * - pos: Position (3D vector)
 * - rot: Rotation (SO3)
 * - offset_R_L_I: LiDAR-IMU rotation offset (SO3)
 * - offset_T_L_I: LiDAR-IMU translation offset (3D vector)
 * - vel: Velocity (3D vector)
 * - bg: Gyroscope bias (3D vector)
 * - ba: Accelerometer bias (3D vector)
 * - grav: Gravity vector (S2 manifold)
 */
...
/**
 * Process noise manifold
 * - ng: Gyroscope noise
 * - na: Accelerometer noise
 * - nbg: Gyroscope bias random walk
 * - nba: Accelerometer bias random walk
 */
...
/**
 * Get the default process noise covariance matrix
 */
...
/**
 * Process model: state derivative as function of state and input
 */
...
/**
 * Jacobian of process model with respect to noise
 */
...
/**
 * Convert SO3 rotation to Euler angles (roll, pitch, yaw) in degrees
 */
...
// Number of nearest points for plane fitting
